...
const debug = (0, debug_1.default)('vite-plugin-vue-i18n:index');
const INTLIFY_BUNDLE_IMPORT_ID = '@intlify/vite-plugin-vue-i18n/messages';
const installedPkg = (0, bundle_utils_1.checkInstallPackage)('@intlify/vite-plugin-vue-i18n', debug);
...
function pluginI18n(options = { forceStringify: false }) {
    ...
    if (include) {
        if ((0, shared_1.isArray)(include)) {
            include = include.map(item => (0, vite_1.normalizePath)(item));
        }
        else if ((0, shared_1.isString)(include)) {
            include = (0, vite_1.normalizePath)(include);
        }
    }
    const filter = (0, pluginutils_1.createFilter)(include);
    const runtimeOnly = (0, shared_1.isBoolean)(options.runtimeOnly)
        ? options.runtimeOnly
        : true;
    // prettier-ignore
    const compositionOnly = installedPkg === 'vue-i18n'
        ? (0, shared_1.isBoolean)(options.compositionOnly)
            ? options.compositionOnly
            : true
        : true;
    // prettier-ignore
    const fullInstall = installedPkg === 'vue-i18n'
        ? (0, shared_1.isBoolean)(options.fullInstall)
            ? options.fullInstall
            : true
        : false;
    const defaultSFCLang = (0, shared_1.isString)(options.defaultSFCLang)
    ...
    const getAliasName = () => installedPkg === 'petite-vue-i18n' &&
        (0, shared_1.isBoolean)(useVueI18nImportName) &&
    ...
    return {
        ...
        config(config, { command }) {
            if (command === 'build' && runtimeOnly) {
                ...
                if ((0, shared_1.isArray)(config.resolve.alias)) {
        ...
        configResolved(config) {
            ...
            if (jsonPlugin) {
                ...
                jsonPlugin.transform = async function (code, id) {
                     ...
                     */
                    const { query } = (0, query_1.parseVueRequest)(id);
                    if (query.vue) {
                        return;
                    }
        ...
        async load(id) {
            if (getVirtualId(id) === INTLIFY_BUNDLE_IMPORT_ID && include) {
                let resourcePaths = [];
                const includePaths = (0, shared_1.isArray)(include) ? include : [include];
                for (const inc of includePaths) {
                    resourcePaths = [...resourcePaths, ...(await (0, fast_glob_1.default)(inc))];
                }
        ...
        async transform(code, id) {
            ...
            if (!query.vue) {
                ...
                if (isCustomBlock(query)) {
                    if ((0, shared_1.isString)(query.lang)) {
                        langInfo = query.src
                            ? query.lang === 'i18n'
                                ? 'json'
                                : query.lang
...
function isCustomBlock(query) {
    ...
    return (!(0, shared_1.isEmptyObject)(query) &&
        'vue' in query &&
        (query['type'] === 'custom' ||
            query['type'] === 'i18n' ||
